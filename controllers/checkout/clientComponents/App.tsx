...
import {
    getQueryStr,
    username,
} from '../../template/clientComponents/TemplateContext';
import {
    getItems,
    getUser,
    listPush,
...
const Return = () => {
    ...
    useEffect(() => {
        ...
        async function push() {
            try {
                const r = (await $$$('/data/api/purchase/POST.php', listPush())
                    .api()
                    .post()) as Response;
                if (r.success) {
        ...
        return (
            <div className="flex justify-center items-center p-[40px] flex-col w-screen h-screen">
                <div className="w-[100px] mb-[40px]">
                    <img
                        draggable={false}
                        src="/controllers/client/img/done.png"
                        alt="payment_complete"
                    />
                </div>
                <section id="success">
                    ...
                    <p className="text-center text-[20px]">
                        Going to your purchased template in {count} second
                        {count <= 1 ? '' : 's'}
                    </p>
                </section>
            </div>
