...
const CountryCodeDropDown = ({ isListShown, listRef, setDropDown }: Props) => {
    ...
    function defaultValue(): void {
        switch (name) {
            ...
            case 'HotLine':
                if (
                    data['HotLineCode'] === null ||
                    data['HotLineCode'] === '' ||
                    data['HotLineFlag'] === null ||
                    data['HotLineFlag'] === ''
                ) {
                    setCountryCode({ code: '+1', flag: 'us' });
                }
            ...
            case 'Whatsapp':
                if (
                    data['WhatsappCode'] === null ||
                    data['WhatsappCode'] === '' ||
                    data['WhatsappFlag'] === null ||
                    data['WhatsappFlag'] === ''
                ) {
                    setCountryCode({ code: '+1', flag: 'us' });
                }
    ...
    const worker = new Worker(
        '/controllers/client/src/dist/admin/clientComponents/CountryCode/worker.js'
    );
    ...
    return (
        isListShown && (
            <div
                style={top}
                className="flex codeList !absolute flex-col left-0 h-[500%] bg-white rounded-[10px] z-[999] p-[5px]"
                ref={listRef}
            >
                ...
                <div className="codeList__list overflow-auto m-[5px]">
                    {list.map((country) => (
                        <div
                            onClick={(e) => countryCodeClickHandler(e, country)}
                            key={country.code}
                            className="each flex flex-row justify-between p-[5px] cursor-pointer rounded-[10px] hover:bg-[#d9d9d9]"
                            data-flag={country.code}
                        >
                            <p>{country.name}</p>
