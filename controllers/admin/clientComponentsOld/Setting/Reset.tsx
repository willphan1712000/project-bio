...
const Reset = () => {
    ...
    async function handleClick() {
        ...
        $('.template_name').css(
            'font-size',
            parseInt(result?.data.fontSize.replace('px', '')) + 15
        );
    }
    ...
    return (
        <>
            <button
                onClick={handleClick}
                className="reset flex justify-center items-center flex-shrink-0 cursor-pointer h-fit typebox"
            >
                <i className="fa-solid fa-circle-arrow-left mr-[5px]"></i> Reset
            </button>
        </>
    );
};
