...
import {
    ...
    Options,
    RangeSlider,
} from '../../../client/src/Web-Development/W';
...
const Setting = ({ data, css, resource }: Props) => {
    ...
    useEffect(() => {
        ...
        document.addEventListener('click', (e) => {
            ...
            if (
                document.querySelector('.background')?.contains(ele) ||
                document.querySelector('#W_colorPicker')?.contains(ele) ||
                document
                    .querySelector('#colorOptionsInBackground')
                    ?.contains(ele)
            ) {
                dispatch({ type: 'background' });
            } else if (
                document.querySelector('.font')?.contains(ele) ||
                document.querySelector('#fontOptions')?.contains(ele)
            ) {
                dispatch({ type: 'font' });
            } else if (
                document.querySelector('.fontSize')?.contains(ele) ||
                document.querySelector('#W_rangeSlider')?.contains(ele)
            ) {
                dispatch({ type: 'fontSize' });
            } else if (
                document.querySelector('.fontColor')?.contains(ele) ||
                document.querySelector('#colorOptions')?.contains(ele)
            ) {
    ...
    return (
        <AdminContextProvider
            data={data}
            css={css}
            regex={resource.regexMap}
            label={resource.labelMap}
                ...
                <div id="setting_board" className="flex gap-1">
                    {state.background &&
                        (background ? (
                            <>
                                <ColorPickerGradient
                                    keyValue="W_colorPicker"
                                    defaultColor={css.background}
                                    cb={(color) => {
                                ...
                                />
                                <Options
                                    keyValue="colorOptionsInBackground"
                                    Face={ColorType}
                                    list={solidColors}
                                    cb={(color) => {
                                        css.background = color as string;
                            ...
                            <div
                                className="flex items-center justify-center size-full rounded-[2rem] bg-white mx-[1rem] text-center"
                                style={{
                                    boxShadow:
                            ...
                            >
                                <p>
                                    This template does not allow background
                                    change
                                </p>
                            </div>
                        ))}
                    ...
                    {state.fontSize && (
                        <RangeSlider
                            keyValue="W_rangeSlider"
                            range={{ min: 0, max: 25 }}
                            defaultValue={parseInt(
                                css.fontSize.replace('px', '')
                            )}
                            cb={(value) => {
                    ...
                    {state.input && (
                        <AdminElementContext.Provider value={state.inputName}>
                            <div
                                className="w-full bg-white rounded-[20px] mx-4 p-1"
                                id="inputElement"
                            >
                                <SocialTag />
                            </div>
...
const SettingBar = memo(({ resource }: { resource: Resource }) => {
    return (
        <div
            id="setting_bar"
            className="[&::-webkit-scrollbar]:hidden flex flex-row gap-[10px] p-[10px] items-center overflow-auto"
