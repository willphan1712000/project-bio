    ...
    if (state.show)
        return (
            <div className="backdrop-blur-lg w-screen h-[100dvh] fixed top-0 left-0 p-4 flex flex-col justify-center items-center">
                <div
                    className="absolute top-[10px] right-[10px] px-3 py-2 text-xs font-medium text-center text-black bg-[#d4d4d4] rounded-lg cursor-pointer"
                    onClick={() => {
                        dispatch({ type: 'show' });
                        $('body').css({
                            overflow: 'auto',
                ...
                {Object.keys(items).length !== 0 ? (
                    <>
                        <p className="text-center w-full p-5 text-[30px]">
                            Your Cart
                        </p>
