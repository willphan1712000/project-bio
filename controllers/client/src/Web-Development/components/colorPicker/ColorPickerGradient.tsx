...
const ColorPickerGradient = ({ keyValue, defaultColor, cb }: Props) => {
    ...
    function colorOneChange(e: React.ChangeEvent<HTMLInputElement>) {
        ...
        setColor(
            `linear-gradient(${deg}deg, ${hslToHex(Number(color2), 100, 80)}, ${hslToHex(Number(color1), 100, 80)})`
        );
    }
    ...
    const colorInputClass =
        'appearance-none h-[10px] rounded-[20px] bg-[linear-gradient(to_right,_hsl(0,_100%,_50%),_hsl(60,_100%,_50%),_hsl(120,_100%,_50%),_hsl(180,_100%,_50%),_hsl(240,_100%,_50%),_hsl(300,_100%,_50%),_hsl(360,_100%,_50%))] outline-none bg-opacity-90 border-none';

    return (
        <div
            id={keyValue}
            className="flex flex-row justify-center items-center h-auto bg-white rounded-[20px] p-[10px] z-[1] flex-1"
        >
