...
export default class Transform {
    ...
    constructor(ele1: HTMLElement, ele2?: HTMLElement, ele3?: string) {
        ...
        if (this.ele1 === null) {
            throw new Error(
                'wrapper elemenet is not defined or not rendered yet'
            );
        }
    ...
    public transform(): Transform {
        ...
        const handleDrag = (event: any, type: 'desk' | 'touch') => {
            ...
            if (type === 'desk') {
                controllerWrapper.addEventListener(
                    'mousemove',
                    eventMoveHandler,
                    false
                );
                window.addEventListener(
                    ...
                    function eventEndHandler() {
                        controllerWrapper.removeEventListener(
                            'mousemove',
                            eventMoveHandler,
                            false
                        );
        ...
        var leftTop = document.querySelector(
            '.' + this.controllerClassName + ' .resize-topleft'
        ) as HTMLElement;
        var rightTop = document.querySelector(
            '.' + this.controllerClassName + ' .resize-topright'
        ) as HTMLElement;
        var rightBottom = document.querySelector(
            '.' + this.controllerClassName + ' .resize-bottomright'
        ) as HTMLElement;
        var leftBottom = document.querySelector(
            '.' + this.controllerClassName + ' .resize-bottomleft'
        ) as HTMLElement;
        ...
        const resizeHandler = (
            ...
            mousedownCb(event);
            var vectorC = [
                mousePressX - initX - this.imgFrame.offsetLeft,
                mousePressY - initY - this.imgFrame.offsetTop,
            ];
            const eventMoveHandler = (event: any) => {
                var x =
                    type === 'desk' ? event.clientX : event.touches[0].clientX;
        ...
        leftTop.addEventListener('mousedown', (e) =>
            resizeHandler(e, true, true, true, true, 'desk')
        );
        rightTop.addEventListener('mousedown', (e) =>
            resizeHandler(e, false, true, true, true, 'desk')
        );
        rightBottom.addEventListener('mousedown', (e) =>
            resizeHandler(e, false, false, true, true, 'desk')
        );
        leftBottom.addEventListener('mousedown', (e) =>
            resizeHandler(e, true, false, true, true, 'desk')
        );

        leftTop.addEventListener(
            'touchstart',
            (e) => resizeHandler(e, true, true, true, true, 'touch'),
            { passive: true }
        );
        rightTop.addEventListener(
            'touchstart',
            (e) => resizeHandler(e, false, true, true, true, 'touch'),
            { passive: true }
        );
        rightBottom.addEventListener(
            'touchstart',
            (e) => resizeHandler(e, false, false, true, true, 'touch'),
            { passive: true }
        );
        leftBottom.addEventListener(
            'touchstart',
            (e) => resizeHandler(e, true, false, true, true, 'touch'),
            { passive: true }
        );

        // handle rotation
        var rotate = document.querySelector(
            '.' + this.controllerClassName + ' .rotate'
        ) as HTMLElement;
        ...
        rotate.addEventListener(
            'mousedown',
            (e) => handleRotate(e, 'desk'),
            false
        );
        rotate.addEventListener('touchstart', (e) => handleRotate(e, 'touch'), {
            passive: true,
        });
