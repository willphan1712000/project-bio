...
const Front = () => {
    ...
    const { error, data, isLoading } = useAppQuery(
        'template_user',
    ...
    if (data?.template.isActive) {
        return (
            <div
                className={`flex p-[${template_padding}px] w-fit`}
                ref={front_template_ref}
            >
                <div
                ...
                >
                    ...
                    {Object.keys(data.template_info).map(
                        (field_ori: string) => {
                            let field;
                            ...
                            const info =
                                data.user_info[
                                    field as keyof typeof data.user_info
                                ];
                            if (!info) return;

                            const template_info =
                                data.template_info[
                                    field_ori as keyof typeof data.template_info
                                ];
                            ...
                            return (
                                <div
                                    ...
                                    dangerouslySetInnerHTML={{
                                        __html: isAdmin
                                            ? template_info.isIcon
                                                ? info.htmlAdmin
                                                : info.htmlAdminWValue
                                            : template_info.isIcon
                                              ? info.html
                                              : info.htmlWValue,
                                    }}
