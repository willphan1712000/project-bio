...
const StatCards = () => {
    const {
        ...
        data: analyticsInfo,
        error,
    } = useAppQuery('analytics', apiDashboard.analytics);
    useAppEffect(error);
    ...
    return (
        <div className="flex flex-col items-center md:flex-row gap-5 py-10">
            <StatCard
                ...
                value={analyticsInfo?.numberOfUsers}
            />
            <StatCard
                color="two"
                icon={<SiMoneygram size="30" />}
                title="Pro templates"
                value={analyticsInfo?.numberOfTemplates}
            />
            <StatCard
                color="five"
                icon={<GrMoney size="30" />}
                title="Total subscription"
                value={analyticsInfo?.numberOfSubscriptions}
            />
        </div>
    );
};
