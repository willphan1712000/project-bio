...
const Users = () => {
    const {
        ...
        data: users,
        error,
    } = useAppQuery('users', apiUsers.getUsers);
    ...
    const handleDeleteUser = async (username: string) => {
        ...
        if (!res) {
            toast(<AppToaster message="Delete unsuccessfully" />);
        } else {
            toast(<AppToaster message="Delete successfully" status={true} />);
        }
    };
                ...
                <Table.Body>
                    {users?.map((user) => (
                        <Table.Row key={user.username}>
                            <Table.RowHeaderCell>
                                <a href={`/${user.username}`} target="__blank">
                                    {user.username}
                                </a>
                            </Table.RowHeaderCell>
