...
const PricingModel = () => {
    ...
    const [pricing, setPricing] = useState<PricingModel[] | undefined>(
        undefined
    );
    ...
    const {
        ...
        data: pricingQuery,
        error,
    } = useAppQuery('pricing', apiPricing.get);
    const { mutateAsync: pricingUpdate } = useAppMutation(
        'pricing',
        apiPricing.post
    );
    useAppEffect(error);
    ...
    const handleUpdate = async () => {
        ...
        if (error) {
            return toast(<AppToaster message={error} />);
        }
            ...
            >
                <div className={containerClasses}>
                    ...
                    <div className="my-5 bg-white p-[1px] rounded-full w-fit">
                        <Button size="3" onClick={handleUpdate}>
                            Update
                        </Button>
                    </div>
                </div>
