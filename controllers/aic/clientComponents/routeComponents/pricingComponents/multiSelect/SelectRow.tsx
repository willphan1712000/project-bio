...
const SelectRow = ({ id }: Props) => {
    ...
    return (
        <div className="flex flex-col md:flex-row gap-6 relative w-full items-center my-[10px]">
            ...
            <AppInput title="Price ($)" name="price" id={id} />
            <AppInput title="Discount (%)" name="discount" id={id} />
            <AppInput title="Period (Month)" name="period" id={id} />
            <div className="relative flex-1 w-full">
                <span className="absolute top-[-22px] left-0">
                    Is Recurring?
                </span>
                <Switch
                    size="3"
            ...
            <div
                className="cursor-pointer border-[2px] border-transparent hover:border-[red] rounded-full z-10 transition-all"
                title="delete row"
